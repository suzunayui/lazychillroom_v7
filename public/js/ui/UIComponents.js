...
class UIComponents {
    ...
    static debugServerIcons() {
        ...
        serverItems.forEach((item, index) => {
            ...
            console.log(`  アイコンHTML: ${icon ? icon.innerHTML : 'なし'}`);
            
            if (img) {
                console.log(`  画像ソース: ${img.src}`);
                console.log(`  画像状態: ${img.complete ? '読み込み完了' : '読み込み中'}`);
                console.log(`  画像表示: ${img.style.display === 'none' ? '非表示' : '表示'}`);
                console.log(`  フォールバック: ${img.getAttribute('data-fallback')}`);
            } else {
                console.log(`  テキストアイコン: ${icon ? icon.textContent : 'なし'}`);
            }
        });
